---
title: データサイエンス100本ノック（構造化データ加工編） - Ruby (RedAmber) - 解答例付き
...
注）　%i は要素をシンボルとした配列を返すRubyの%記法です。詳しくは、[Ruby 3.2 リファレンスマニュアル>リテラル>%記法](https://docs.ruby-lang.org/ja/latest/doc/spec=2fliteral.html#percent)を参照して下さい。RedAmberでは、列の名前は、文字列、シンボルのどちらでも受け付け、引数としては配列でも配列を展開したリテラルでもどちらでも受け付けます。下記(1),(2),(3),(4)はどれも有効です。

```{ruby}
# df_receipt.pick(:sales_ymd, :customer_id, :product_cd, :amount) # (1)
# df_receipt.pick('sales_ymd', 'customer_id', 'product_cd', 'amount') # (2)
# keys = [:sales_ymd, :customer_id, :product_cd, :amount]
# df_receipt.pick(keys) # (3)
# df_receipt.pick(*keys) # (4)
```
...
注）　RedAmberでは & は Vectorクラスのメソッドで、各要素の論理積をベクトルで返しますので、trueの要素だけフィルタリングすることができます。Rubyでよくある条件式の和　&&　ではないことに注意が必要です。また、演算子として & の優先順位は　&& のように低くないので、括弧が必要です。詳しくは
[Ruby 3.2 リファレンスマニュアル>演算子式](https://docs.ruby-lang.org/ja/latest/doc/spec=2foperator.html)を参照して下さい。

### RA-006. 複数条件に合致する行を抽出する (2)
...
注） 先に :amount順にソートして、１0件取り出しておくと処理が速いです。 #rank のデフォルトは、小さい順にランクをつける（:ascending)、同一順位も出現順に順位付けする(:first)なので、上記のようにオプション指定します。
なお、RedAmber 0.5.0以前では #rank でオプションが使えませんでした。 また、0.4.2以前では #rankそのものがありませんでした。Arrow CGLibにRankOptionsを追加していただきましたので使えるようになりました。
...
df_receipt
  ...
  .sort('-amount')
  .head(10)
  .assign(:amount_rank) { amount.rank(:descending) }
```
...
注）　複数のやり方があります。ベンチマークによる比較を行います。初めて呼ぶときは #count_uniqが速いですが、#unique.sizeでは#sizeを一回呼ぶとメモ化されるため高速で値を返しています。どちらもArrayより高速です。
...
Arrowのgroupingでは、hash_mode()はないので最頻値を求めることはできない。このため、速度は遅いがSubFramesをグルーピングに使い、各要素に`Vector#mode`を使う。

`Vector#mode`は数値型にしか使えない。
参考に、元になっているArrow C++のドキュメントを表示させると下記のようになる。なお、Arrow 12.0.1 (RedAmber 0.5.0) ではまだオプション(ModeOptions)が使えるようになっていない。

```{ruby}
puts ArrowFunction::arrow_doc(:mode)
```

代わりに`Vector#tally`を使い、得られたHashのvalueの最大値を最頻値としている。
...
#| tags: [answer]
df_receipt
  .group(:store_cd) { variance(:amount) }
  .sort('-variance(amount)')
  .head(5)
```

注）　分散は実際は浮動小数点値として得られている。

```{ruby}
df_receipt.group(:store_cd) { variance(:amount) }.v('variance(amount)')
```

RedAmberでは、数値Vectorに対するメソッドとして、`#sd`, `#var`が不偏標準偏差と不偏分散(unbiased, ddof=1)を求めるメソッドである。`#stddev`, `#variance`が標準偏差、分散(biased, ddof=0)である。Group用のメソッドは標準偏差と分散しか用意されておらず、それぞれ `Group#stddev`, `Group#variance`である。
...
```{ruby}
#| tags: [answer]
df_receipt.amount.quantiles
```

個別のパーセンタイル値を求める場合は下記のようにする。
...
注）　`#remove`は`#group`の後で行っても良く、速度的にもその方が有利ですが、問題文の流れに沿って解答を組むと後からremoveする形になるでしょう。結果として答えは同じになります。RedAmberでは、メソッドチェーンの入れ替えが簡単にできますので、リファクタリングが容易です。
...
`DataFrame#inner_join` は引数に取ったデータフレームと内部結合を行う。第二引数 join_key を省略した場合は共通する名前を自動的に使う。
...
```{ruby}
#| tags: [answer]
valid_receipt = df_receipt.remove { customer_id.start_with('Z') }

member_group =
  valid_receipt
    .tap { p _1.amount.is_nil.any}
    .tap { p (_1.amount == 0).any }
    .group(:customer_id)
```
...
customer_days =
  member_group
    .summarize(:sales_days) { count_uniq(:amount) }
    .sort('-sales_days')
    .head(20)
```
...
wide =
  df_customer
    ...
    .assign(:age_group) { (age / 10.0).floor * 10 }
    .inner_join(receipt)
...
#| tags: [answer]
df_receipt
  .pick(:receipt_no, :receipt_sub_no)
...
customer =
  df_customer
    ...
    .assign(:is_tokyo) do
      postal3, _ = postal_cd.split_to_columns('-')
      postal3 = postal3.cast(:uint16)
...
df =
  df_product
    .pick(%i[product_cd unit_price unit_cost])
...
df_monday =
  DataFrame.new(
    ...
    .assign do
      weekday = sales_date.map { _1.wday }
      n_days = (weekday == 0).if_else(6, weekday - 1)
      {
        last_monday: sales_date.zip(n_days).map { |d, n| d - n },
        elapsed_days: n_days,
      }
    end
```
...
filled =
  df_product
    .assign do
      {
        unit_price: unit_price.fill_nil(unit_price.mean.round),
        unit_cost: unit_cost.fill_nil(unit_cost.mean.round),
      }
    end
filled.tdr
```
...
filled =
  df_product
    .assign do
      {
        unit_price: unit_price.fill_nil(unit_price.median.round),
        unit_cost: unit_cost.fill_nil(unit_cost.median.round),
      }
    end
filled.slice(indices)
```
...
df_median =
  df_product
    .group(:category_small_cd)
    .median(:unit_price, :unit_cost)
```
...
filled =
  df_product
    ...
    .assign do
      {
        unit_price: unit_price.fill_nil(v('median(unit_price)').round),
        unit_cost: unit_cost.fill_nil(v('median(unit_cost)').round),
      }
    end
    .drop('median(unit_price)', 'median(unit_cost)')
filled.slice(indices)
```
...
df_customer_year =
  df_receipt
    .pick(:customer_id, :sales_ymd, :amount)
    .assign(:sales_year) { sales_ymd / 10000 }
    .group(:customer_id, :sales_year)
    .summarize(:amount) { sum(:amount) }
```
...
```{ruby}
total = []
df_customer.head(10).customer_id.each do |id|
  t = df_customer_totals.then { _1.amount[_1.customer_id == id] }
  total << (t.nil? ? 0 : t)
end
total
```
...
df_by_2019 =
  DataFrame.new(customer_id: df_customer.customer_id.uniq)
    .assign(:amount_2019, :amount_total, :ratio) do
      a2019 = []
      total = []
      ratio = []
      customer_id.each do |id|
        i = df_customer_year.then { (_1.customer_id == id) & (_1.sales_year == 2019) }
        a = df_customer_year.amount[i]
        a = (a.nil? ? 0 : a)
        a2019 << a
        t = df_customer_totals.then { _1.amount[_1.customer_id == id] }
        t = (t.nil? ? 0 : t)
...
```{ruby}
#| tags: []
df_geocode.group(:postal_cd).count.sort('-count')
```
...
```{ruby}
#| tags: [answer]
customer = df_customer.inner_join(mean_geocode, :postal_cd)
customer.head(10)
```
...
```{ruby}
#| tags: [answer]
joined = customer.inner_join(df_store, join_keys = {left: :application_store_cd, right: :store_cd})
...
joined
  .assign(:distance) do
    long1 = v(:'mean(longitude)') * Math::PI / 180
    long2 = longitude * Math::PI / 180
    lat1 = v(:'mean(latitude)')  * Math::PI / 180
...
df_aggregated =
  df_customer
    .left_join(df_customer_amount, :customer_id)
    .assign(:amount) { amount.fill_nil(0) }
...
duplicated =
  df_aggregated
    ...
    .count
    .slice { count > 1 }
    .assign(:is_duplicated) { [true] * size }
    .drop(:count)
```

```{ruby}
df = df_aggregated.left_join(duplicated, :customer_name_postal_cd)
index_at_top_amount =
  df.slice { is_duplicated }
    .pick(:customer_name_postal_cd, :amount)
    .assign(:index) { _1.indices }
    .sub_by_value(:customer_name_postal_cd)
    .assign(:rank) { amount.rank(:descending) }
    .concatenate
    .sort('index')
    .remove { _1.rank != 1 }
```
...
df_duplicated =
  df.slice { is_duplicated }
    .slice(index_at_top_amount)
df_normalized =
  df.remove { is_duplicated }
    .concatenate(df_duplicated)
```
...
puts "名寄せ顧客データ件数: #{df_normalized.size}"
...
```{ruby}
original = df_customer.customer_id.count_uniq
unified = df_normalized.customer_id.count_uniq
puts "顧客IDのユニーク件数: #{original}"
puts "統合名寄IDのユニーク件数: #{unified}"
puts "差: #{original - unified}"
```
...
df_all
  ...
  .each.map do |df|
    df.build_subframes { _1.indices.partition { |i| i < train_size } }.to_a
  end
```
